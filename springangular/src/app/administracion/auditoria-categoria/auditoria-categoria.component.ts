...
@Component({
  selector: 'app-auditoria-categoria',
  standalone: true,
    ...
    <div class="min-h-screen bg-slate-950 text-white font-sans flex">
      ...
      <main class="flex-1 p-10">
        <div class="flex justify-between items-center mb-6 border-b border-slate-900 pb-6">
          <div>
            <h1 class="text-3xl font-black text-blue-400">Auditoría de Categorías</h1>
            <p class="text-slate-400 text-sm mt-1">Cambios en categorías por almacenero (HU-33).</p>
          </div>
          <span class="material-symbols-outlined text-4xl text-teal-500">policy</span>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-800 text-sm">
              <tr *ngFor="let log of logs()" class="hover:bg-slate-800/20 transition-colors">
                <td class="p-4 pl-6 font-mono text-xs text-blue-400">#{{ log.id_auditoria_categoria }}</td>
                <td class="p-4">
                  <div class="text-white font-medium">{{ log.nombre }}</div>
                  <div class="text-xs text-slate-500">ID {{ log.id_categoria }}</div>
                </td>
                ...
                <td class="p-4 pr-6 text-right text-xs text-slate-500 font-mono">{{ log.fecha_actualizacion | date:'dd/MM/yyyy HH:mm' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class AuditoriaCategoriaComponent implements OnInit {
  logs = signal<any[]>([]);
  loading = signal(true);
  f: AuditoriaFiltro = {};
  ...
  filtrar() {
    ...
    this.auditoriaService.filtrarCategorias(this.f).subscribe({

      ...
      <main class="flex-1 p-10">
        <div class="flex justify-between items-center mb-8 border-b border-slate-900 pb-6">
          <div>
            ...
            <p class="text-slate-400 text-sm mt-1">Bitácora histórica de creación, edición y eliminación de categorías.</p>
          </div>
          <span class="material-symbols-outlined text-4xl text-rose-500">policy</span>
        </div>
        ...
        <div *ngIf="loading" class="p-10 text-center text-slate-500 flex flex-col items-center justify-center gap-2">
          ...
          <span>Cargando bitácora de categorías...</span>
        </div>
        ...
        <div *ngIf="!loading && logs.length === 0" class="p-20 text-center text-slate-500 flex flex-col items-center justify-center gap-3">
          ...
          <span>No hay registros de auditoría de categorías disponibles.</span>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-850 text-sm">
              <tr *ngFor="let log of logs" class="hover:bg-slate-800/20 transition-colors">
                ...
                <td class="p-4 font-mono text-xs text-slate-300">#{{ log.id_categoria }}</td>
                <td class="p-4">
...
export class AuditoriaCategoriaComponent implements OnInit {
  ...
  ngOnInit() {
    this.auditoriaService.getAuditoriaCategorias().subscribe({
      next: (data) => {

...
import com.pry.demo.model.*;
import com.pry.demo.repository.*;
...
public class PedidoController {
    ...
    public ResponseEntity<?> createPedido(@RequestBody Pedido pedidoRequest) {
        ...
        final List<Map<String, Object>> detallesParaEmail = new ArrayList<>(); // Para el ticket por email
        ...
        // 2. Guardar Detalles con el ID del pedido
        ...
        // 4. Enviar Email (Opcional: En un hilo separado para no bloquear la respuesta)
        try {
            emailService.sendOrderTicket(user, pedidoGuardado, detallesParaEmail);
        } catch (Exception e) {
            System.err.println("Error asíncrono capturado: " + e.getMessage());
        }
    ...
    public ResponseEntity<?> getPedidoDetails(@PathVariable Long id) {
        ...
        // Enriquecer con nombres de productos para el recibo

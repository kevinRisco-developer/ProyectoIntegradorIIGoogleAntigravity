...
public interface AuditoriaProductoRepository extends JpaRepository<AuditoriaProducto, Long> {
    ...
    @Query("SELECT a FROM AuditoriaProducto a WHERE " +
            "(:desde IS NULL OR a.fecha_modificacion >= :desde) AND (:hasta IS NULL OR a.fecha_modificacion <= :hasta) AND " +
            "(:responsable IS NULL OR a.id_almacenero = :responsable) AND " +
            "(:accion IS NULL OR a.accion = :accion) " +
            "ORDER BY a.fecha_modificacion DESC, a.id_auditoria_producto DESC")
    List<AuditoriaProducto> filtrar(@Param("desde") Timestamp desde, @Param("hasta") Timestamp hasta,
